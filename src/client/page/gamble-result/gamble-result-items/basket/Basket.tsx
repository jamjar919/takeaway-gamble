...
const Basket: React.FC<BasketProps> = (props) => {
  const { selectedItems, ctaUrl, imageUrl } = props;

  const total =
    selectedItems
      .map((selectedItem) => {
        const itemPrice = getPriceFromDeliverooObject(
          selectedItem.item
        ).fractional;
  ...
  return (
    <>
      {imageUrl && (
        <img src={imageUrl} className={styles.image} alt={"Restaurant image"} />
      )}
      <div className={styles.basket}>
        ...
        <ul className={styles.basketItems}>
          {selectedItems
            .sort(
              (a, b) =>
                getPriceFromDeliverooObject(b.item).fractional -
                getPriceFromDeliverooObject(a.item).fractional
            )
            .map((selectedItem) => (
              <BasketItem
        ...
        <div className={styles.row}>
          <div className={styles.banner}>
            <span>
              View the code on{" "}
              <a href="https://github.com/jamjar919/deliveroo-gamble">Github</a>
            </span>
            <span>
              Created by{" "}
              <a href="https://thejamespaterson.com">James Paterson</a>
            </span>
          </div>
        </div>
        <div className={styles.row}>
          <div className={styles.totalLine}>
            ...
            <div className={styles.price}>{locale.format(total)}</div>
          </div>
          <a
